...
class AuxiliaryBlock(nn.Module):
    def __init__(self, in_channels, num_classes=100):
        ...
        self.conv_one_by_one = ConvBlock(in_channels, 128, kernel_size=1)
...
class QuickNet_GoogLeNet(nn.Module):

    def __init__(self, in_channels=3, use_auxiliary=True, num_classes=100):
        super(QuickNet_GoogLeNet, self).__init__()
        ...
        if use_auxiliary:
            self.auxiliary4a = AuxiliaryBlock(512, num_classes)
            self.auxiliary4d = AuxiliaryBlock(528, num_classes)
